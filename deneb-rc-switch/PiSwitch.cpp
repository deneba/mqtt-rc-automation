...
extern "C" { 
  ...
  void sendCommand(const char* hexCode, unsigned int length, unsigned int proto)
  {
    unsigned long Code;
    RCSwitchSender *piSwitch = new RCSwitchSender();
    Code = strtol(hexCode,NULL,16);
    piSwitch->enableTransmit(0);
    piSwitch->setProtocol(proto);
    piSwitch->sendEasyHome(Code,length);
  }
}
...
void RCSwitchSender::sendEasyHome(unsigned long Code, unsigned int length) {
	rcSwitch->sendEasyHome(Code, length);
}

void RCSwitchSender::sendRFWirelessDimmer(unsigned long Code, unsigned int length) {
	rcSwitch->sendRFWirelessDimmer(Code, length);
}
