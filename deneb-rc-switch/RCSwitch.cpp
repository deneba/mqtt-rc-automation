...
void RCSwitch::sendRFWirelessDimmer(unsigned long Code, unsigned int length) {
  char* sCodeWord = this->dec2binWzerofill(Code, length);
  // First 4 sets with short sync
...
void RCSwitch::sendEasyHome(unsigned long Code, unsigned int length) {
  char* sCodeWord = this->dec2binWzerofill(Code, length);
  // First 4 sets with short sync
  ...
  // Last 4 sets with long sync
  for (int nRepeat=0; nRepeat<4; nRepeat++) {
    this->sendeasyhomelongsync();
    int i = 0;
    while (sCodeWord[i] != '\0') {
      switch(sCodeWord[i]) {
        case '0':
...
void RCSwitch::sendeasyhomelongsync() {

    this->transmit(8,19);
}  

void RCSwitch::transmitHigh(int nHighPulses) {
    #if not defined ( RCSwitchDisableReceiving )
